...
4. **Sound cues** — ✅ **done**. No sound assets existed and none can be recorded or downloaded with license guarantees, so the 4 tones are self-synthesized sine waves (`scripts/gen-sounds.mjs`, pure Node, no deps) written as WAV to `assets/sounds/`: `tick.wav` (each countdown number), `go.wav` (distinct, lower/longer, at 0), `minute.wav` (EMOM minute change), `end.wav` (double beep — AMRAP reaching 0, or EMOM's last minute). Installed `expo-audio` via `expo install` (works in Expo Go; it auto-added itself to `app.json`'s `plugins`, no other config needed). New `hooks/use-sound-cue.ts` wraps `useAudioPlayer` in a stable, fire-and-forget `play()` callback (rewinds via `seekTo(0)` first so rapid replays work). Wired in `components/judge/start-overlay.tsx` (countdown) and `hooks/use-judge.ts` (EMOM minute-change effect from B3 now also fires `end` on the final minute; a new small effect fires `end` once when `elapsed` reaches `totalSeconds` for `timerMode: 'remaining'` formats, i.e. AMRAP today). No mute toggle: relies on the device's media volume, so it's silent when the device is (Android media volume, not a ring/silent switch as on iOS).
